...
#define TRIG_PIN 2   // 輸入：MRI Trigger
...
// --- 寫入Flash的核心函式
void saveToFlashBuffer(unsigned long timestamp, byte type, byte state) {
...
void dumpFlashToSD() {
  ...
  if (dataFile) {
    ...
    while (readAddr < currentFlashAddr) {
      flash.readByteArray(readAddr, readBuf, BUFFER_SIZE);
      
      for(int i=0; i<BUFFER_SIZE; i+=EVENT_SIZE) {
        unsigned long ts = (unsigned long)readBuf[i] << 24 | (unsigned long)readBuf[i+1] << 16 | (unsigned long)readBuf[i+2] << 8 | readBuf[i+3];
        byte type = readBuf[i+4];
        byte state = readBuf[i+5];
    ...
    // 2. 別忘了還有留在 Arduino RAM Buffer 裡沒湊滿一頁的剩餘資料
    for (int i = 0; i < bufferIndex; i += EVENT_SIZE) {
...
void setup() {
  ...
  // 重要：實驗開始前先確認 Flash 目前使用到哪裡，如果還有很多額外空間就把初始address往後挪，已經快滿就清空 (Erase Chip)
...
void loop() {
  ...
  int inputState = digitalRead(TRIG_PIN); // 讀取這次的數據
  ...
  switch (currentState) {
    case IDLE:
      if (trig_flag) {
        Serial.println(F("Trigger detected — starting sequence..."));
        //counter = 0;
        trig_flag = false;
        cycle_count = 0;
